...
public sealed class MediaListFactory : IMediaListFactory
{
    ...
    private async Task<List<MediaViewModel>> ParseSubMediaAsync(MediaViewModel source, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var subItems = media.SubItems
                .Where(IsLocalSubMedia)
                .Select(item => _mediaFactory.GetTransient(item));
    ...
    private static bool IsLocalSubMedia(Media media)
    {
        return !Uri.TryCreate(media.Mrl, UriKind.Absolute, out var uri) || IsLocalFileUri(uri);
    }
