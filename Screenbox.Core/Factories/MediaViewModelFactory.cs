...
public sealed class MediaViewModelFactory
{
    ...
    {
        return new MediaViewModel(_playerContext, _playerService, _settingsService, _thumbnailService, _thumbnailLoadingService, file);
    }

    public MediaViewModel GetTransient(Uri uri)
    {
        return new MediaViewModel(_playerContext, _playerService, _settingsService, _thumbnailService, _thumbnailLoadingService, uri);
    }

    public MediaViewModel GetTransient(Media media)
    {
        if (!Uri.TryCreate(media.Mrl, UriKind.Absolute, out Uri uri))
            return new MediaViewModel(_playerContext, _playerService, _settingsService, _thumbnailService, _thumbnailLoadingService, media);

        // Prefer URI source for easier clean up
        MediaViewModel vm = new(_playerContext, _playerService, _settingsService, _thumbnailService, _thumbnailLoadingService, uri)
        {
    ...
    public MediaViewModel GetSingleton(StorageFile file)
    {
        ...
        instance = new MediaViewModel(_playerContext, _playerService, _settingsService, _thumbnailService, _thumbnailLoadingService, file);
    ...
    public MediaViewModel GetSingleton(Uri uri)
    {
        ...
        // No existing reference, create new instance
        instance = new MediaViewModel(_playerContext, _playerService, _settingsService, _thumbnailService, _thumbnailLoadingService, uri);
        if (!string.IsNullOrEmpty(id))
        {
            _references[id] = new WeakReference<MediaViewModel>(instance);
            CleanUpStaleReferences();
