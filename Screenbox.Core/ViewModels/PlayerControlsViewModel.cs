    ...
    IRecipient<PropertyChangedMessage<PlayerVisibilityState>>
{
    ...
    public bool IsCurrentItemFavorite => Playlist.CurrentItem?.IsFavorite == true;

    public bool IsCustomThumbnailEnabled => _settingsService.UseCustomVideoThumbnails;
    ...
    [ObservableProperty]
    [NotifyCanExecuteChangedFor(nameof(SetCustomThumbnailCommand))]
    ...
    public void Receive(SettingsChangedMessage message)
    {
        switch (message.SettingsName)
        {
            ...
            case nameof(SettingsPageViewModel.PlayerShowChapters):
                ...
                break;
            case nameof(SettingsPageViewModel.UseCustomVideoThumbnails):
                OnPropertyChanged(nameof(IsCustomThumbnailEnabled));
                SetCustomThumbnailCommand.NotifyCanExecuteChanged();
                break;
        }
    }
    ...
    private void PlaylistViewModelOnPropertyChanged(object sender, PropertyChangedEventArgs e)
    {
        switch (e.PropertyName)
        {
            case nameof(MediaListViewModel.CurrentItem):
                ...
                OnPropertyChanged(nameof(IsCurrentItemFavorite));
    ...
    {
        MediaViewModel? media = Playlist.CurrentItem;
    ...
    {
        MediaViewModel? media = Playlist.CurrentItem;
        if (media == null || !_favoritesContext.IsLoaded) return;
        media.IsFavorite = _favoritesContext.Favorites.Any(f => SameLocation(f, media));
    }
    ...
    private bool CanSetCustomThumbnail()
    {
        return HasVideo && IsCustomThumbnailEnabled;
    }

    [RelayCommand(CanExecute = nameof(CanSetCustomThumbnail))]
    private async Task SetCustomThumbnailAsync()
    {
        if (MediaPlayer?.PlaybackState is not (MediaPlaybackState.Paused or MediaPlaybackState.Playing)) return;
        MediaViewModel? current = Playlist.CurrentItem;
        if (current == null) return;

        try
        {
            byte[] imageBytes = await CaptureCurrentFrameAsync(MediaPlayer);
            await current.SetCustomThumbnailAsync(imageBytes);
            Messenger.Send(new ThumbnailCacheInvalidatedMessage());
            Messenger.Send(new CustomThumbnailSetNotificationMessage(current.DisplayName));
        }
        catch (Exception e)
        {
    ...
    private static async Task<byte[]> CaptureCurrentFrameAsync(IMediaPlayer mediaPlayer)
    {
        ...
        try
        {
            ...
            StorageFile file = await WaitForSnapshotAsync(tempFolder);
    ...
    private static async Task<StorageFile> WaitForSnapshotAsync(StorageFolder folder)
    {
        DateTimeOffset deadline = DateTimeOffset.Now + TimeSpan.FromSeconds(5);
        while (DateTimeOffset.Now < deadline)
        {
            StorageFile? file = (await folder.GetFilesAsync()).FirstOrDefault();
            if (file != null)
            {
                return file;
            }
        ...
        throw new Exception("VLC did not create a snapshot file");
    }
}
