...
namespace Screenbox.Core.Playback
{
    public sealed class VlcMediaPlayer : IMediaPlayer
    {
        ...
        public bool IsMuted
        {
            ...
            set
            {
                _targetIsMuted = value;
                if (!_startupAudioGuardActive && VlcPlayer.Mute != value)
                {
        ...
        public double Volume
        {
            ...
            set
            {
                _targetVolume = Math.Max(0, value);
                if (_startupAudioGuardActive) return;

                int iVal = (int)(_targetVolume * 100);
        ...
        public PlaybackItem? PlaybackItem
        {
            ...
            set
            {
                ...
                if (value == null)
                {
                    _startupAudioGuardActive = false;
                    _startupAudioGuardVersion++;
                    VlcPlayer.Stop();
        ...
        private bool _startupAudioGuardActive;
        private int _startupAudioGuardVersion;
        private double _targetVolume = 1;
        private bool _targetIsMuted;
        ...
        private void BeginStartupAudioGuard()
        {
            _startupAudioGuardActive = true;
            _startupAudioGuardVersion++;
            VlcPlayer.Mute = true;
            VlcPlayer.Volume = 0;
        }

        private async void CompleteStartupAudioGuard()
        {
            if (!_startupAudioGuardActive) return;

            int version = _startupAudioGuardVersion;
            int targetVolume = (int)(_targetVolume * 100);
            VlcPlayer.Volume = targetVolume;

            // LibVLC can briefly recreate the audio output as media starts, so give it a
            // short moment before unmuting to prevent a full-volume startup pop.
            await Task.Delay(150);
            if (!_startupAudioGuardActive || version != _startupAudioGuardVersion) return;

            VlcPlayer.Volume = targetVolume;
            VlcPlayer.Mute = _targetIsMuted || targetVolume == 0;
            _startupAudioGuardActive = false;
        }
