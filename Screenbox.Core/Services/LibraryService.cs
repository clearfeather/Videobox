...
public sealed class LibraryService : ILibraryService
{
    ...
    private const string VideoFolderTokensKey = "VideoBox/VideoFolderTokens";
    private const char VideoFolderTokenSeparator = '\n';
    ...
    public async Task<IReadOnlyList<StorageFolder>> GetVideoLibraryFoldersAsync()
    {
        string[] tokens = GetSavedVideoFolderTokens();
        List<StorageFolder> folders = new();
        List<string> validTokens = new();
        ...
        foreach (string token in tokens)
        {
            ...
            {
                // Token no longer resolves; drop it from VideoBox's local folder list.
            }
        }

        if (validTokens.Count != tokens.Length)
        {
            SaveVideoFolderTokens(validTokens);
        }

        return folders;
    }

    public async Task<StorageFolder?> AddVideoLibraryFolderAsync()
    {
        ...
        List<StorageFolder> existingFolders = (await GetVideoLibraryFoldersAsync()).ToList();
        if (existingFolders.Any(existing =>
                string.Equals(existing.Path, folder.Path, StringComparison.OrdinalIgnoreCase)))
        {
            return folder;
        }

        List<string> tokens = GetSavedVideoFolderTokens().ToList();
        string token = StorageApplicationPermissions.FutureAccessList.Add(folder);
        tokens.Add(token);
        SaveVideoFolderTokens(tokens);
        return folder;
    }

    public async Task RemoveVideoLibraryFolderAsync(StorageFolder folder)
    {
        string[] tokens = GetSavedVideoFolderTokens();
        List<string> keepTokens = new();

        foreach (string token in tokens)
        {
            try
            {
                StorageFolder savedFolder = await StorageApplicationPermissions.FutureAccessList.GetFolderAsync(token);
                if (string.Equals(savedFolder.Path, folder.Path, StringComparison.OrdinalIgnoreCase))
                {
            ...
            {
                // Token is stale; do not keep it.
            }
        }

        SaveVideoFolderTokens(keepTokens);
    }
    ...
    {
        var folderPaths = context.VideoFolders.Select(f => f.Path).ToList();
    ...
    {
        if (context.VideoFolders.Count == 0)
        {
            context.Videos = new List<MediaViewModel>();
        ...
        try
        {
            ...
            if (useCache)
            {
                ...
                if (libraryCache?.Records.Count > 0)
                {
                    ...
                    hasCache = !AreFolderPathsChanged(libraryCache.FolderPaths, context.VideoFolders);
                }
            }
            ...
            if (!hasCache)
            {
                context.Videos = videos;
                videos.Clear();
                foreach (StorageFolder folder in context.VideoFolders)
                {
                    StorageFileQueryResult libraryQuery = CreateVideoFolderQuery(folder, _settingsService.UseIndexer);
                ...
                if (SearchRemovableStorage)
                {
                    ...
                    if (accessStatus is KnownFoldersAccessStatus.Allowed or KnownFoldersAccessStatus.AllowedPerAppFolder)
                    {
                        StorageFileQueryResult libraryQuery = CreateRemovableStorageVideosQuery();
                        await BatchFetchMediaAsync(libraryQuery, videos, cancellationToken);
                    }
                }
            }
    ...
    private async Task BatchFetchMediaAsync(StorageFileQueryResult queryResult, List<MediaViewModel> target, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            List<MediaViewModel> batch = await FetchMediaFromStorage(queryResult, fetchIndex);
            if (batch.Count == 0) break;
            fetchIndex += (uint)batch.Count;
    ...
    private static bool AreFolderPathsChanged(IReadOnlyCollection<string> cachedFolderPaths, IReadOnlyCollection<StorageFolder> folders)
    {
    ...
    private static StorageFileQueryResult CreateVideoFolderQuery(StorageFolder folder, bool useIndexer)
    {
        QueryOptions queryOptions = new(CommonFileQuery.OrderByName, FilesHelpers.SupportedVideoFormats)
        {
            IndexerOption = useIndexer ? IndexerOption.UseIndexerWhenAvailable : IndexerOption.DoNotUseIndexer
    ...
    private static string[] GetSavedVideoFolderTokens()
    {
        if (ApplicationData.Current.LocalSettings.Values.TryGetValue(VideoFolderTokensKey, out object value) &&
            value is string serialized &&
            !string.IsNullOrWhiteSpace(serialized))
        {
            return serialized.Split(new[] { VideoFolderTokenSeparator }, StringSplitOptions.RemoveEmptyEntries);
        }

        return Array.Empty<string>();
    }

    private static void SaveVideoFolderTokens(IEnumerable<string> tokens)
    {
        string serialized = string.Join(VideoFolderTokenSeparator.ToString(), tokens.Where(token => !string.IsNullOrWhiteSpace(token)));
        ApplicationData.Current.LocalSettings.Values[VideoFolderTokensKey] = serialized;
    }
